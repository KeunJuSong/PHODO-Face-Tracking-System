...
class FaceBoundsOverlay @JvmOverloads constructor(ctx: Context, attrs: AttributeSet? = null) :
    ...
    init {
        anchorPaint.color = ContextCompat.getColor(context, android.R.color.holo_purple)

        idPaint.color = ContextCompat.getColor(context, android.R.color.holo_purple)
        idPaint.textSize = 40f

        boundsPaint.style = Paint.Style.STROKE
        boundsPaint.color = ContextCompat.getColor(context, android.R.color.holo_purple)
        boundsPaint.strokeWidth = 8f
    }
